...
-   Differences between Bayesian and Frequentist
    -   Also see
        -   [Why I love Bayesian statistics for developmental research](https://www.apadivisions.org/division-7/publications/newsletters/developmental/2018/07/bayesian-statistics)
...
-   It's a good idea to plot your priors to get a sense of the assumptions in your model.
    -   [Example]{.ribbon-highlight}: `curve(dunif( x, 0, 50), from = -10,  to = 60)`  for σ \~ Uniform( 0, 50 ) constrains the σ to have a positive probability from 0 to 50 cm for a height outcome variable
...
-   Uncertainty
    ...
    -   Interpretation of HDPI when x = x0
        -   [Example]{.ribbon-highlight}: Given a person's weight = 50kg, what does an 89% HDPI around our model's height prediction mean?
...
-   Confidence Interval - repeating the calculation for every x0 will give you the regression line CI
    -   [Example]{.ribbon-highlight} brms regression line CI
...
-   Prediction Interval
    ...
    -   [Example]{.ribbon-highlight} PI and CI around regression line![](./_resources/Untitled_Note.resources/unnamed-chunk-101-1.png)
...
-   Splines (2nd Edition)
    ...
    -   [Example]{.ribbon-highlight}: Temperature vs Time![](./_resources/Untitled_Note.resources/Screenshot%20(87).png)
    ...
    -   [Example]{.ribbon-highlight}: 15 knots at 5 quantiles, cubic basis functions![](./_resources/Untitled_Note.resources/Screenshot%20(88).png)
...
-   Counterfactual plots - Shows the implied predictions for imaginary experiments in which the different predictor variables can be changed independently of one another
    ...
    -   [Example]{.ribbon-highlight}: How does Divorce Rate (D) (outcome) change when we change Median Age at Marriage (predictor) values? Assumed causal model: ![](./_resources/Untitled_Note.resources/Screenshot%20(98).png)
...
-   [Example]{.ribbon-highlight}: What is the expected causal effect of increasing median age at marriage from 20 to 30?
...
-   [Example]{.ribbon-highlight}: How does Divorce Rate (outcome) change when we change values of Marriage Rate (predictor)? The arrow A → M is deleted, because if we control the values of M, then A no longer influences it ![](./_resources/Untitled_Note.resources/Screenshot%20(108).png)
...
-   Visualizing contrasts
    ...
    -   [Example]{.ribbon-highlight}: Posterior Mean Contrast (1 predictor) (From [Lecture 4](https://youtu.be/QiHKdvAbYII)) ![](./_resources/Untitled_Note.resources/Screenshot%20(709).png)
    ...
    -   [Example]{.ribbon-highlight}: Posterior Predictive Contrast (1 predictor) (From [Lecture 4](https://youtu.be/QiHKdvAbYII))![](./_resources/Untitled_Note.resources/Screenshot%20(708).png)
...
-   [Example]{.ribbon-highlight}: Posterior Mean Contrast vs Predictor (2 predictors) (From [Lecture 4](https://youtu.be/QiHKdvAbYII))![](./_resources/Untitled_Note.resources/Screenshot%20(704).png)
    -   Weight was the response with height and sex as predictors
...
-   3 of the potential problems are discussed
    -   Multicollinearity
        ...
        -   Solutions
            -   Identifying and *automatically* dropping highly correlated predictors is a mistake. Pairwise correlations are not the problem. It is the conditional associations---not correlations---that matter. And even then, the right thing to do will depend upon what is causing the collinearity. The associations within the data alone are not enough to decide what to do.
                -   [Example]{.ribbon-highlight}: Thinking the situation through causally in order to deal with multicollinearity![](./_resources/Untitled_Note.resources/Screenshot%20(113).png)
    ...
    -   Post-Treatment Bias
        ...
        -   [Example]{.ribbon-highlight} (adjusting for F masks the treatment effect)
...
-   [Example]{.ribbon-highlight} (adjusting for F unmasks the treatment effect)
...
-   Collider Bias
    -   When you condition on a collider, it induces statistical---but not necessarily causal--- associations among its causes.
        -   Although, the variables involved may be useful for predictive models as the backdoor paths do provide valid information about statistical associations within the data
    -   [Example]{.ribbon-highlight} (non-misleading)![](./_resources/Untitled_Note.resources/Screenshot%20(120).png)
    ...
    -   [Example]{.ribbon-highlight} (misleading)![](./_resources/Untitled_Note.resources/Screenshot%20(121).png)
...
-   Specification
    -   McElreath says his indexing approach and the traditional way of specifying an interaction are equivalent
        ...
        -   [Example]{.ribbon-highlight}: ruggedness x gid (cont x cat) interaction (also see below)
...
-   Metropolis adjusted Langevin algorithm (MALA) and Hamiltonian Monte Carlo (HMC) have improved efficiency over non-gradient based methods such as Random Walk Metropolis (RWM), but these methods exhibit decreased robustness to tuning of the algorithm’s parameters. ([source](https://joss.theoj.org/papers/10.21105/joss.08594))
    -   *The Barker proposal* provides a middle road. It offers similar efficiency in high-dimensional settings as other gradient-based methods such as MALA (Vogrinc et al., 2023), while allowing easier adaptive tuning of the algorithm’s parameters. It also provides improved robustness to certain forms of irregularity, such as skewness, in the target distribution
...
-   Non-identifiable parameters
    ...
    -   [Example]{.ribbon-highlight} (bad) priors: a1 \~ dnorm( 0 , 1000 ), a2 \~ dnorm( 0 , 1000 ) ![](./_resources/Untitled_Note.resources/Screenshot%20(193).png)
    -   Solution
...
-   [Example]{.ribbon-highlight}: 0/1 logistic regression
...
-   **Absolute Effects**: The effect of a (counter-factual) change in predictor value (type of treatment) has on the outcome (*probability* of an event)
    ...
    -   Base rate matters
        -   [Example]{.ribbon-highlight} (fictional)
...
-   **Relative Effects**: The effect of a (counter-factual) change in predictor value (type of treatment) has on the outcome (*odds* of an event)
    ...
    -   Can be conditionally very important, when baseline rates of predictor variables change.
        -   [Example]{.ribbon-highlight}: Conditional on being in the ocean, sharks are much more dangerous than bees. So in this situation the relative risk is more important than the absolute risk.
...
-   [Example]{.ribbon-highlight}: Binomial regression (*balanced* number of trials)
...
-   [Example]{.ribbon-highlight}: Binomial regression (*unbalanced* number of trials)
...
-   [Example]{.ribbon-highlight}: UCB Admissions Binomial Regression (lecture 9 [video](https://www.youtube.com/watch?v=nPi5yGbfxuo))
...
-   [Example]{.ribbon-highlight}: Counts with varying rates![](./_resources/Untitled_Note.resources/Screenshot%20(10).png)
...
-   `predict` / `posterior_predict` - gives predicted values and PIs by default; "simulated values" (same as SR `sim`)
    ...
    -   For details on all these predict function, also see
        -   Heiss [article](https://www.andrewheiss.com/blog/2022/09/26/guide-visualizing-types-posteriors/)
        -   mc-stan forum [post](https://discourse.mc-stan.org/t/confusion-on-difference-between-posterior-epred-and-posterior-predict-in-a-mixed-effects-modelling-context/28813)
